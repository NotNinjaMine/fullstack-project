...
**Roles:** Authenticated user (self only)

Accepts only `phone` and `address`; employment, role, department, office, and country fields are HR-managed.

### GET /api/leave/balance
**Roles:** All authenticated users

Returns the balance fields plus `ai_summary`. The summary is `null` when no AI provider is configured or the request times out.

### POST /api/admin/holidays/load
**Roles:** HR Admin

```json
{ "country_code": "SG", "year_from": 2030, "year_to": 2040 }
```

Loads and caches the requested range. The system has no forward year ceiling; 2030+ searches and applications prefetch the next ten years in the background.

### Leave-request overrides
`POST /api/leave` accepts nullable `applicant_name_override` and `applicant_department_override`. `GET /api/leave/:id` returns these values in `applicant` when present, otherwise the current staff profile values.

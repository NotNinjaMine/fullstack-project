      ...
      {optional.length > 0 && (
        <div className="max-w-6xl mx-auto px-4 pt-4 space-y-2">
          {optional.map((a) => (
            <div
              key={a.id}
              className="flex items-start gap-3 rounded-xl border border-brand-200 bg-brand-50 px-4 py-3 shadow-lf-sm"
            >
              <Megaphone className="w-5 h-5 text-brand-700 flex-shrink-0 mt-0.5" />
              <div className="flex-1">
                <p className="font-medium text-brand-900">{a.title}</p>
                <p className="text-sm text-brand-800 whitespace-pre-line">{a.body}</p>
                <p className="text-xs text-brand-600 mt-1">From {a.createdByName}</p>
              </div>
              <button
                type="button"

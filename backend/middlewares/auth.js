...
// Verifying the JWT signature alone is NOT enough. A JWT stays valid until it
// expires (30 days), so on signature alone a revoked session, a locked account
// or a deactivated employee would keep working until then - which would make
// "force logout", "lock" and "deactivate" purely cosmetic.
//
...
const validateToken = async (req, res, next) => {
    try {
        ...
        // No session row means the token predates this session (or the session
        // was deleted with the account) - treat it as signed out.
        if (!session) return res.sendStatus(401);
        if (session.revokedAt) return res.sendStatus(401);

...
const { sendResetEmail, smtpConfigured, emailEnabled } = require('../services/mailer');
...
const publicUser = (user) => ({
    ...
    // UC-10: needed alongside country to resolve gender-restricted leave types
    // (e.g. maternity, NS/reservist leave) — see routes/leaveRequest.js.
    gender: user.gender,
    team: user.team,
...
router.post("/register", async (req, res) => {
    ...
    let validationSchema = yup.object({
        ...
        team: yup.string().oneOf(TEAMS).default(DEFAULT_TEAM)
    });
    try {
...
router.post("/login", async (req, res) => {
    ...
    try {
        ...
        // M1 (UC-25): account lockout. Two things land here, and they need
        // different wording: a 3-failed-password lockout auto-expires in 15
        // minutes, while an admin "Sign out" (lockReason=ADMIN) sets a
        // far-future expiry and only clears when an admin unlocks. Telling
        // someone in the second case to "try again after <a date 100 years
        // out>" would be actively misleading.
        if (session.isLocked(user)) {
            return res.status(423).json({
                message: user.lockReason === "ADMIN"
                    ? "This account has been signed out and locked by an administrator. Ask HR or your Manager to unlock it."
                    : `Account locked after too many failed attempts. Try again after ${new Date(user.lockedUntil).toLocaleTimeString("en-SG")}, or ask HR to unlock.`
            });
        }
        ...
        // never yields an access token — we open a challenge and offer the configured
        // email delivery path. The code is generated only after the user requests
        // it in POST /user/2fa/send.
        //
        // TWO_FACTOR_MODE=optional reverts to per-user opt-in (user.twoFactorEnabled)
        // if you ever want single-step logins for accounts that haven't enabled it.
        const twoFactorRequired = (process.env.TWO_FACTOR_MODE || "always").toLowerCase() === "always"
...
router.post("/forgot-password", async (req, res) => {
    ...
    try {
        ...
        const demoMode = process.env.NODE_ENV !== 'production' && !emailEnabled();
        const demo = demoMode ? { demoResetToken: token } : {};
        if (smtpConfigured() && !mail.sent) {
            console.error(`[user] reset email delivery failed for user=${user.id}; see sanitized mailer category.`);
        }
...
// The teams that actually exist, for the team pickers on the add-employee and
// invitation forms. Derived from the staff table rather than hard-coded, so a
// new team appears in the dropdown the moment someone is placed in it.
router.get("/teams", validateToken, requireRole("SUPERVISOR", "MANAGER", "HR_ADMIN", "BOSS"), async (req, res) => {
...
router.post("/employees", validateToken, requireRole("SUPERVISOR", "MANAGER", "HR_ADMIN", "BOSS"), async (req, res) => {
    let validationSchema = yup.object({
        name: nameRule,
        email: emailRule,
        ...
        // Closed list: the team dropdown is populated from GET /coverage/options.
        team: yup.string().oneOf(TEAMS, `Team must be one of: ${TEAMS.join(", ")}`).nullable(),
...
router.get("/locked", validateToken, requireRole("MANAGER", "HR_ADMIN", "BOSS"), async (req, res) => {
...
router.put("/:id/unlock", validateToken, requireRole("MANAGER", "HR_ADMIN", "BOSS"), async (req, res) => {
...
router.put("/:id/force-logout", validateToken, requireRole("MANAGER", "HR_ADMIN", "BOSS"), async (req, res) => {
    const user = await User.findByPk(req.params.id);
    if (!user) return res.sendStatus(404);
...
/* ---------------- UC-10: account removal (deactivate / reactivate / delete) ---------------- */
...
router.put("/:id/deactivate", validateToken, requireRole("MANAGER", "HR_ADMIN", "BOSS"), async (req, res) => {
    const user = await User.findByPk(req.params.id);
    if (!user) return res.sendStatus(404);
    if (user.id === req.user.id) {
...
router.put("/:id/reactivate", validateToken, requireRole("MANAGER", "HR_ADMIN", "BOSS"), async (req, res) => {
    const user = await User.findByPk(req.params.id);
    if (!user) return res.sendStatus(404);
    if (user.status !== "DEACTIVATED") {
...
router.delete("/:id", validateToken, requireRole("MANAGER", "HR_ADMIN", "BOSS"), async (req, res) => {
    const user = await User.findByPk(req.params.id);
    if (!user) return res.sendStatus(404);
    if (user.id === req.user.id) {
...
// POST /user/2fa/send { challengeToken, method: "EMAIL" }
// Generates and delivers the email code. Can be called again to resend.
...
// POST /user/2fa/resend { challengeToken } — new code on the same challenge.
router.post("/2fa/resend", async (req, res) => {
    ...
    try {
        const data = await validationSchema.validate(req.body, { abortEarly: false });
        const result = await twoFactor.resendChallenge(data.challengeToken);
        if (!result.ok) return res.status(result.status || 400).json({ message: result.message });
        res.json({
        ...
        if (err.errors) return res.status(400).json({ errors: err.errors });
        res.status(400).json({ message: err.message || "Resend failed." });
    }
});

// GET /user/2fa — the caller's own 2FA status (for the My account screen).
router.get("/2fa", validateToken, async (req, res) => {
    const user = await User.findByPk(req.user.id);
    if (!user) return res.sendStatus(404);
    res.json({
        ...
        emailDeliveryConfigured: require('../services/mailer').smtpConfigured()
    });
});

// POST /user/2fa/setup { method, password } — sends an email code to prove
// the user can receive it. 2FA is not switched on until /2fa/confirm succeeds.
router.post("/2fa/setup", validateToken, async (req, res) => {
    let validationSchema = yup.object({
        method: yup.string().oneOf(["EMAIL"]).required(),
        password: yup.string().trim().min(8).max(50).required()
    });
    try {
        const data = await validationSchema.validate(req.body, { abortEarly: false });
        ...
        // Re-authenticate: changing a security setting must require the password.
        const ok = await bcrypt.compare(data.password, user.password);
        if (!ok) return res.status(400).json({ message: "Password is incorrect." });

        const challenge = await twoFactor.createChallenge(user, req, data.method);
        res.json({
            challengeToken: challenge.challengeToken,
            method: challenge.method,
            destination: challenge.destination,
            delivered: challenge.delivered,
            deliveryError: challenge.deliveryError,
            ...(challenge.demoCode ? { demoCode: challenge.demoCode } : {}),
            message: challenge.delivered
                ? `We sent a code to ${challenge.destination}. Enter it to turn on 2-step verification.`
                : `Enter the code to turn on 2-step verification.`
        });
    } catch (err) {
        if (err.errors) return res.status(400).json({ errors: err.errors });
        res.status(err.status || 400).json({ message: err.message || "Could not start setup." });
    }
});

// POST /user/2fa/confirm { challengeToken, code } — turns 2FA ON.
router.post("/2fa/confirm", validateToken, async (req, res) => {
    let validationSchema = yup.object({
        challengeToken: yup.string().trim().length(64).matches(/^[0-9a-f]+$/).required(),
        code: yup.string().trim().matches(/^\d{6}$/, "code must be 6 digits").required()
    });
    try {
        ...
        const result = await twoFactor.verifyChallenge(data.challengeToken, data.code);
        if (!result.ok) return res.status(result.status || 400).json({ message: result.message });
        // The challenge must belong to the caller.
        if (result.user.id !== req.user.id) return res.sendStatus(403);

        const user = result.user;
        user.twoFactorEnabled = true;
        user.twoFactorMethod = result.challenge.method;
        await user.save();
        await session.logEvent(user.id, "TWO_FACTOR_ENABLED", req.ip, true);

        res.json({
            message: `2-step verification is on. You'll enter a code from your email each time you sign in.`,
            enabled: true,
            method: user.twoFactorMethod
        });
    } catch (err) {
        if (err.errors) return res.status(400).json({ errors: err.errors });
        res.status(400).json({ message: err.message || "Could not confirm." });
    }
});

// POST /user/2fa/disable { password } — turns 2FA OFF (password required).
router.post("/2fa/disable", validateToken, async (req, res) => {
    ...
    try {
        ...
        await user.save();
        await TwoFactorChallenge.update({ consumedAt: new Date() }, { where: { userId: user.id, consumedAt: null } });
        await session.logEvent(user.id, "TWO_FACTOR_DISABLED", req.ip, true);
        res.json({ message: "2-step verification is off.", enabled: false });
    } catch (err) {
        if (err.errors) return res.status(400).json({ errors: err.errors });
        res.status(400).json({ message: err.message || "Could not disable." });
    }
});

// PUT /user/:id/2fa/reset — RECOVERY. If someone loses access to their mailbox
// they would otherwise be permanently locked out, so HR *and* Managers can clear
// the second step for any account (Managers included so a locked-out HR admin is
// still recoverable — same safety net as PUT /user/:id/unlock).
router.put("/:id/2fa/reset", validateToken, requireRole("HR_ADMIN", "MANAGER", "BOSS"), async (req, res) => {
    const user = await User.findByPk(req.params.id);
    if (!user) return res.sendStatus(404);
    user.twoFactorEnabled = false;
    // Clear an authenticator enrolment too: losing the phone that holds the TOTP
    // secret is exactly the case this recovery route exists for, and leaving it
    // on would keep offering a method the user can no longer satisfy.
    user.totpEnabled = false;
    user.totpSecret = null;
    user.totpPendingSecret = null;
    await user.save();
    await TwoFactorChallenge.update({ consumedAt: new Date() }, { where: { userId: user.id, consumedAt: null } });
    await session.logEvent(user.id, "TWO_FACTOR_RESET", req.ip, true);
    res.json({ message: `2-step verification cleared for ${user.name}. They can sign in with an emailed code and set their preferences again.` });
});

...
//
// Flow: password verified -> createChallenge() sends a 6-digit code by email
// and returns an opaque challengeToken -> the browser posts that token plus
// the code to /user/2fa/verify -> only then is a real access token issued.
//
// Security properties:
...
// On-screen 6-digit codes are a DEMO affordance, and they are decided per
...
//
// Production additionally suppresses codes outright, as a belt-and-braces guard.
const { isDemoEmail } = require('./demoEmailDomain');
...
const isDemoAccount = (user, method) => {
    ...
    if (normalizeMethod(method) === "SMS" && user.phone && sms.isDemoPhone(user.phone)) return true;
    return false;
};
...
// Never disclose the full email address back to an unauthenticated caller.
...
// M1 (UC-25) supports email, SMS and an authenticator app. Anything else a
// legacy row may carry falls back to EMAIL, which every account can always use.
const SUPPORTED_METHODS = ["EMAIL", "SMS", "AUTHENTICATOR"];
const normalizeMethod = (method) =>
    SUPPORTED_METHODS.includes(String(method || "").toUpperCase())
...
// STEP 2: generate + send the email code on the existing challenge.
...
const sendCodeForChallenge = async (challengeToken, method) => {
    ...
    if (method === "AUTHENTICATOR") {
        ...
        // Demo accounts don't have anyone actually holding a real phone with
        // this secret enrolled, so — exactly like the email/SMS demo affordance
        // below — show the current code on screen instead of requiring one.
        // Real/live accounts never get this: only a genuine authenticator app
        // ever sees their code.
        const demoCode = isDemoAccount(user, "AUTHENTICATOR")
            ? totp.currentCode(totp.decrypt(user.totpSecret))
            : null;
    ...
    // Demo accounts always get the code on screen — including when the mail DID
    // send, because the demo mailbox has no reader. Real accounts never do.
    const showDemoCode = isDemoAccount(user, method);
    if (showDemoCode) {
        console.log(`[2fa] demo code shown on screen for demo user ${user.id} (${method}).`);
    }
    ...
    return {
        ...
        expiresInSeconds: Math.max(0, Math.floor((new Date(challenge.expiresAt) - Date.now()) / 1000)),
        ...(showDemoCode ? { demoCode: code } : {})
    };
};

// Create + send a challenge in one call. Used by the 2FA SETUP flow (My account),
// where the method is already chosen by the form.
// methodOverride is used by the enable/setup flow, which must send via the
// PROPOSED method before it has been saved on the user.
const createChallenge = async (user, req, methodOverride = null) => {
    const method = normalizeMethod(methodOverride || user.twoFactorMethod);
    const dest = destinationFor(user);
    if (!dest.ok) {
        const err = new Error(dest.message);
        err.status = 400;
        throw err;
    }
    ...
    return {
        ...
        delivered: !!delivery.sent,
        deliveryError: delivery.error || null,
        // Demo accounts only. A real account never receives its own code back.
        ...(showDemoCode ? { demoCode: code } : {})
    };
};
...
// Resend the code on the same challenge (new code, same challengeToken).
const resendChallenge = async (challengeToken) => {
    const challenge = await findLiveChallenge(challengeToken);
    if (!challenge) {
        return { ok: false, status: 400, message: "This verification request is invalid or has expired. Please sign in again." };
    }
    if (challenge.method === "AUTHENTICATOR") {
        return { ok: false, status: 400, message: "Your authenticator app refreshes the code itself every 30 seconds — there is nothing to resend." };
    }
    if (challenge.resendCount >= MAX_RESENDS) {
        return { ok: false, status: 429, message: "Resend limit reached. Please sign in again to start over." };
    }
    if (challenge.lastSentAt && (Date.now() - new Date(challenge.lastSentAt).getTime()) < RESEND_COOLDOWN_SECONDS * 1000) {
        const wait = Math.ceil((RESEND_COOLDOWN_SECONDS * 1000 - (Date.now() - new Date(challenge.lastSentAt).getTime())) / 1000);
        return { ok: false, status: 429, message: `Please wait ${wait}s before requesting another code.` };
    }
    ...
    const method = normalizeMethod(challenge.method);
    const dest = destinationFor(user);
    if (!dest.ok) return { ok: false, status: 400, message: dest.message };
    ...
    challenge.lastSentAt = new Date();
    challenge.attempts = 0; // fresh code, fresh attempt budget
    await challenge.save();

    const delivery = await deliverCode(challenge.method, dest.to, code);
    const showDemoCode = isDemoAccount(user, challenge.method);
    if (showDemoCode) {
        console.log(`[2fa] demo code regenerated on screen for demo user ${user.id} (${challenge.method}).`);
    }
    ...
    return {
        ...
        delivered: !!delivery.sent,
        deliveryError: delivery.error || null,
        resendsLeft: MAX_RESENDS - challenge.resendCount,
        ...(showDemoCode ? { demoCode: code } : {})
    };
};

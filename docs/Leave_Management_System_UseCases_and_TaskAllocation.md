...
> **Revision note — 9 August 2026.** Updated from the original plan to describe
> **what was actually built**. Use-case intent is unchanged; where the build
> extended or diverged from a use case, the section says so explicitly and gives
> the reason. Companion documents: `HLD_LeaveManagementSystem_3.md` (as-built
> design), `docs/architecture.md` (architecture), `docs/<member>/` (per-vertical
> detail).
...
| **HR Admin** | System administrator (Singapore HQ HR). Also decides leadership self-applications and corrects leave already under way. |

> **Built with four roles, not five.** The planned **HOD** oversight tier was not
> implemented. A third approval level added nothing to a two-tier workflow and
> would have complicated every routing rule; HR Admin covers the oversight the HOD
> role was intended to provide. Approval routing is also **team-based** rather
> than following stored reporting lines (`supervisor_id` / `manager_id`) — simpler
> to seed and demonstrate, and `delegationService.assignedApproverId()` is written
> to consume reporting-line fields if they are ever added.
>
> **No one may decide their own request.** A Supervisor's own leave starts at the
> Manager tier; a Manager's or HR Admin's own leave routes to HR Admin, because no
> team peer could decide it without a conflict of interest.
...
2. Selects **Request cancellation**, or **Return early** to keep the leave but end it sooner. Leave type can never be changed — cancel and re-apply instead.
3. If still pending → cancelled immediately; balance untouched (nothing had been deducted).
4. If already approved → the change routes through Supervisor → Manager again.
5. On final approval the balance is corrected.

**As built — three paths, one engine.** The delivered use case is wider than
planned. All three share one shortening engine, differing only in who may act and
when:

| Path | Actor | When | Outcome |
|---|---|---|---|
| **Withdraw pending** | Employee | Before any approval | `CANCELLED` immediately, balance untouched |
| **Withdraw approved** | Employee → Supervisor → Manager | Before the leave starts | `CANCELLED`, **all** days returned |
| **Return early** *(added)* | Employee → Supervisor → Manager | Before the leave starts | Stays `APPROVED`, end date moves, **only the difference** returned |
| **HR adjustment** *(added)* | HR Admin | Including leave **already under way** | Shortened or voided immediately, reason mandatory, audited |

**Why "return early" was added.** Coming back early from a five-day holiday is the
common real case. Cancelling the whole request and re-applying would have lost the
approval and mis-stated the days actually taken.

**Why HR adjustment was added.** The employee cancellation route refused leave that
had already started and told the employee to "ask HR to adjust it instead" — and no
such HR screen existed. The message pointed at a door that was not there.

**Business rules (as built):**
- Only approved leave can be shortened; the new end date must fall inside the original range.
- Trimming only weekends or public holidays returns nothing and is refused with that explanation.
- Trimming every working day is refused and the employee is redirected to full cancellation.
- A half-day request cannot be shortened.
- Once leave has **started**, only HR may change it.
- A cancellation frees coverage rather than reducing it, so it never requires a coverage-exception acknowledgement.
- The state change runs under a database row lock, so a cancellation racing an approval cannot both land.

**Data model.** Two columns on `leave_requests` encode which change is in flight:
`cancellationRequested` alone means "withdraw everything"; together with
`pendingEndDate` it means "end it on that date". This reuses the existing two-tier
approval path instead of creating a second pipeline to keep in step.
...
1. On December 31st at 23:59 Singapore Time (SGT, UTC+8) — the HQ timezone — the system runs a scheduled job (node-cron).
   *(As built: a daily sweep on Singapore time performs this, and HR can also trigger it manually with a preview-then-confirm step.)*
...
1. When applying for sick leave, the employee uploads a medical certificate (PDF or image).
2. The system stores it against the request; the MC vs no-MC quota is applied automatically per country.
3. The approver and HR can view the attachment from the request detail; other staff cannot.
4. **HR can list every sick leave still missing a certificate that policy requires**, and chase it.

**As built:**
- The certificate can be **added or replaced after submission**, while the request is still open. Sick leave is often filed before the certificate is in hand, and the alternative was cancelling and re-applying.
- Whether a certificate is required is driven by the leave type's `requiresMc` flag in HR's catalogue, not a hard-coded list — HR can add an MC-backed type without a code change.
- Files are stored **in-row as base64 data URLs** rather than in a separate file-storage service, which keeps the app self-contained. Consequence: list endpoints must explicitly exclude the column, or every dashboard load ships megabytes.
- **`GET /leave/mc-compliance`** *(added)* lists outstanding certificates, flagged either because the type always requires one (`TYPE_REQUIRES_MC`) or because the absence ran past what self-declaration covers (`EXCEEDS_SELF_DECLARATION`). The document itself is never included — only the fact it is missing.

**Business Rules:**
- Accepted types PDF/JPG/PNG; **~5 MB** limit (the JSON body limit is 12 MB, since base64 inflates a file by about a third).
- Validation is **server-side**. The browser's `accept` attribute is a convenience, not a control.
- Only the request owner, that request's team approvers, and HR may view the document.
- No further uploads once the request is decided.
- Self-declaration is permitted up to **2 consecutive days**; beyond that a certificate is expected. *(Currently a named constant in `leaveRules.js`; it belongs in per-country policy configuration — see Scope Notes.)*
...
### UC-31: Forfeiture Risk Alert in Leave Application System

**Primary Actor:** Employee / HR Admin / Manager

**Main Flow:**

1. The system monitors each employee’s remaining leave balance and the leave policy expiry date.
2. When an employee has leave days that are likely to expire unused, the system identifies them as being at forfeiture risk.
3. The employee sees a notification in the leave application system showing the number of leave days at risk and the expiry date.
4. The employee can select the notification to view suggested dates and submit a leave application.
5. HR Admins and Managers can view a report of employees with leave balances at risk of forfeiture.
6. The system sends reminder notifications at configured intervals until the employee applies for leave, the leave is used, or the balance expires.

**Business Rules:**

- Forfeiture risk applies only to leave types and balances subject to expiry under the organisation’s leave policy.
- The system must calculate risk based on approved leave, pending leave applications, carried-forward leave, and leave expiry dates.
- Employees must only see their own leave risk information; Managers and HR Admins view information according to their access permissions.
- Notifications must stop once the employee’s remaining at-risk leave balance is fully utilised, adjusted, encashed, or expired.
- HR may configure the warning period, such as 30, 60, or 90 days before expiry.
...
1. The system reviews the organisation’s holiday calendar, weekends, employee work schedule, and available leave balance.
2. The system identifies upcoming public holidays that can be combined with one or more leave days to form an extended break.
3. Eligible long-weekend opportunities are displayed in the leave application system, showing the proposed leave dates, total days off, and number of leave days required.
4. The employee selects an opportunity and is taken to a pre-filled leave application form.
5. The employee reviews the application, submits it, and the request follows the standard approval workflow.
6. HR Admins can maintain the public holiday calendar and configure which leave types may be used for long-weekend suggestions.

**Business Rules:**

- Suggestions must consider the employee’s assigned work calendar, including non-standard working days and shifts.
- The system must not suggest dates that have already passed or overlap with approved leave applications.
- A long-weekend opportunity must comply with leave policy, including minimum notice periods, blackout dates, and maximum consecutive leave limits.
- The suggestion is advisory only; submitting leave remains subject to normal approval and leave-balance validation.
- Employees must only see opportunities available within their own location’s public holiday calendar and work schedule.
---

## 6. Task Allocation for 5-Person Team (Full-Stack Verticals)

Each member owns one unique role end-to-end — building both the frontend and the backend for their own area. Enhanced features are marked **(E)** — build Core first and layer Enhanced in Phase 3.

Three use cases are **shared** between two members, split along a clear seam rather than by use case number:
- **UC-09** — Jervis owns the shared UI component library; Jordon owns the responsive layout framework.
- **UC-10** — Jordon owns employee records and CSV staff import; Nabil owns leave-type and country-policy configuration.
- **Database schema ownership & migrations** sits with Nabil (alongside their policy/config work), so every member's table changes are reviewed in one place.
- **UC-04** (year-end carry-forward) sits with Jordon, alongside the other entitlement and onboarding logic in that vertical.

### Workload Summary

| Member | Role | Use Cases | AI Feature | UC Count |
|---|---|---|---|---|
| Jordon | Platform, Identity & Self-Service Engineer | UC-04, UC-09 (responsive layout), UC-10 (employee records & staff import), UC-20, UC-23, UC-24, UC-25, UC-26 | — | 8 |
| Jervis | Employee Leave Experience Engineer | UC-01, UC-03 *(extended)*, UC-05, UC-08 (staff), UC-09 (component library), UC-13 *(extended)*, UC-14, UC-27 | AI-1 | 8 |
| Waiyan | Approval, Delegation & Notification Engineer | UC-02, UC-08 (approver), UC-12, UC-15, UC-16, UC-28 | AI-3 | 6 |
| Wei Jun | Coverage, Calendar & Scheduling-Rules Engineer | UC-06, UC-07, UC-17, UC-18, UC-19, UC-29 | AI-2 | 6 |
| Nabil | HR Admin, Analytics & Automation Engineer | UC-10 (leave-type & policy config), UC-11, UC-21, UC-22, UC-30 | AI-4, AI-5 | 5 |

**Delivered beyond the plan.** Three additions came out of integration testing,
each closing a gap the plan had not anticipated:

| Addition | Owner | Why |
|---|---|---|
| Partial cancellation ("return early") + HR leave adjustment | Jervis (M2) | UC-03 told employees to ask HR, and HR had no screen for it |
| MC compliance list for HR | Jervis (M2) | UC-13 collected certificates but nothing surfaced the missing ones |
| Submission idempotency + transactional approval decisions | Waiyan (M3) | A retried submit created duplicates; concurrent approvals could lose a balance deduction |
| Leave-type catalogue with per-country and gender rules | Nabil (M5) | Lets HR add maternity / NS leave without a code change |

---

### Jordon — Platform, Identity & Self-Service Engineer
*Use cases: UC-04 · UC-09 (responsive layout) · UC-10 (employee records & staff import) · UC-20 · UC-23 · UC-24 · UC-25 · UC-26*

**Frontend:**
- Login / logout & forgot-password screens
- Role-based navigation shell & protected routing
- View switcher in the app header — lets Supervisors / Managers / HR Admin move between their own role page and the employee "Apply for leave" page
- Responsive layout framework (UC-09) — the shared component library itself is owned by Jervis; see UC-09 split below
- Bulk yearly entitlement / pro-ration screen (UC-20)
- Year-end carry-forward preview & confirm screen — per-employee carried / forfeited / new-entitlement breakdown before committing (UC-04)
- Employee record add / edit / deactivate form + CSV staff import (UC-10)
- Employee self-service profile & password change (UC-23)
- Notification-preferences & locale switcher (UC-23, E)
- Multi-language UI — translated My account / profile screens, switching live as the language is picked (UC-23, E)
- Invitation send form + new-employee registration & onboarding tour (UC-24)
- Invitation list with resend (UC-24)
- Standalone account-creation page opened directly from the invitation link (UC-24)
...
- Year-end carry-forward batch job — SGT sweep, delivered with `setInterval` rather than node-cron (UC-04)
- Active-leave-year service — single source of truth for "the current leave year", so the staff table, HR dashboard, balance views and carry-forward reports all agree the moment a year-end rollover is applied (UC-04, UC-10)
- Employee management API + CSV staff import (UC-10)
- Profile & notification-preference APIs (UC-23)
- Internationalisation (i18n) resource backend — string dictionaries with English fallback; each user's locale is returned with their session so the UI renders in their language from first paint (UC-23, E)
- Invite token generation + expiry + registration API (UC-24)
- Invitation resend endpoint (UC-24)
- Session table, revoke endpoint & failed-login lockout (UC-25)
- Announcements table, targeting engine & acknowledgement API (UC-26)
- Shared SGT date helper — announcement display windows (and delegation date ranges) are evaluated against the Singapore business day rather than server UTC (UC-26)

---

### Jervis — Employee Leave Experience Engineer
*Use cases: UC-01 · UC-03 · UC-05 · UC-08 (staff) · UC-09 (component library) · UC-13 · UC-14 · UC-27 · AI-1*

**Frontend:**
- Shared UI component library — buttons, inputs, cards, modals, tables; consumed by every member's screens (UC-09)
...
### Waiyan — Approval, Delegation & Notification Engineer
...
### Wei Jun — Coverage, Calendar & Scheduling-Rules Engineer
...
### Nabil — HR Admin, Analytics & Automation Engineer
*Use cases: UC-10 (leave-type & policy config) · UC-11 · UC-21 · UC-22 · UC-30 · UC-31 · UC-32 · AI-4 · AI-5*

**Frontend:**

- HR admin panel — leave-type & policy configuration (UC-10)
...
- Forfeiture-risk dashboard and employee leave-expiry alert management (UC-31, E)
- Long-weekend opportunity configuration and holiday-calendar management (UC-32, E)
- AI-4 chatbot UI

**Backend:**

- Database schema ownership & migrations — every member's table changes land here for review
- Leave-type & policy management API (UC-10)
...
- Forfeiture-risk calculation service, notification scheduling, and HR/manager reporting API (UC-31, E)
- Holiday-calendar and work-schedule evaluation API; long-weekend opportunity generator with pre-filled leave-request data (UC-32, E)
...
- UC-08 (role-based views) is enforced centrally by Jordon's RBAC layer; each member renders their own persona's calendar and history within those permissions.
- UC-09 is split: Jervis owns the shared UI component library, Jordon owns the responsive layout framework. Settle the component props/API contract early, since every member's screens consume it.
- UC-10 is split: Jordon owns employee records and staff import, Nabil owns leave-type and country-policy configuration. Both write to the same HR admin area, so the panel's tab boundaries need to stay agreed.
- Database schema ownership & migrations sit with Nabil — every member designs the tables their vertical needs, but changes are raised with Nabil so migrations stay in one place and no two members alter the same table blind.
- The working-day & holiday-aware calculation (Wei Jun, UC-19) reads weekend config (UC-29) and is the single source of truth for leave duration — Jervis and Nabil call it via a shared service rather than reimplementing it.
- Country leave-policy rules (Wei Jun) feed the carry-forward (Jordon, UC-04) and sick-leave logic (Jervis) — agree the policy data contract early.
- Anything that reads a leave balance — staff table, HR dashboard, reports, an employee's own balances — must resolve the current year through Jordon's active-leave-year service, or those screens will disagree with each other after a year-end rollover.
- The team-calendar component (Wei Jun) is reused by the employee view (Jervis) and the approver view (Waiyan) — expose it as a shared component with a clear props/API contract.
- Notification preferences (Jordon, UC-23) are read by the notification service (Waiyan, UC-12) — settle the event-type enum up front.
- The invitation flow (Jordon, UC-24) triggers the pro-ration logic (Jordon, UC-20) on account activation — both are self-contained within Jordon's vertical.
- Comment thread notifications (Waiyan, UC-28) follow the same notification service pattern as approval notifications — no extra work for Waiyan.

---

## 7. Database Schema (As Built)

> The block below was the *recommended* schema. The delivered schema differs:
> **MySQL** rather than PostgreSQL, **Sequelize camelCase** column names rather
> than snake_case, **no `hod_id`** (the HOD role was not built), and **no separate
> `attachments` table** (certificates are stored on the request row). Reporting-line
> columns (`supervisor_id`, `manager_id`) were not implemented — routing is
> team-based. The authoritative, verified schema is in
> `HLD_LeaveManagementSystem_3.md` §5 and `docs/jervis/database-schema.md`.
>
> **22 tables were delivered.** Notable columns added during the build:
> `leave_requests.pendingEndDate` (early return), `.cancellationRequested`,
> `.submissionKey` (idempotency), `.stageEnteredAt` (reminder ageing);
> `leave_types.applicableCountries` + `.genderRestriction`;
> `users.gender`, `.totpSecret`, `.failedLoginCount`, `.lockedUntil`.

### 7.1 Original recommendation (superseded — kept for traceability)
...
## 8. Phased Build Plan — planned vs delivered

**All three planned phases were completed, plus a fourth that was not planned.**

| Phase | Planned | Outcome |
|---|---|---|
| 1 — Foundation & Core Flow | Auth, schema, apply/approve, calendar | Delivered |
| 2 — Workflow Depth & Intelligence | Delegation, notifications, coverage, AI-1..AI-3 | Delivered |
| 3 — HR, Analytics & Enhanced Polish | HR admin, reporting, AI-4/AI-5, all (E) features | Delivered |
| **4 — Integration & Hardening** *(unplanned)* | — | **Required.** See below |

### Phase 4 — Integration & Hardening (what actually happened)

The team worked in parallel and exchanged zip files rather than sharing a
repository, so the five verticals were not combined until the end. Merging them
was a phase in its own right, and it surfaced defects that **no member could have
found alone**, because each one lived in the seam between two people's work:

| Defect | Seam | Consequence if missed |
|---|---|---|
| `validateToken` dropped the `gender` field | M1's middleware ↔ M5's gender-restricted leave types | Maternity and NS/reservist leave invisible to **everyone** |
| Five undefined identifiers across three pages | Merge of three versions of the same page | Dead buttons — "Save as draft", "Add leave type" and the draft-edit dialog silently did nothing |
| `services/ai.js` bypassed M3's hardened `llmClient` | M2's AI-1 ↔ M3's provider layer | No timeout, no response sanitising, raw provider errors reaching users |
| Duplicate React state declaration | M1's team list ↔ M4's closed team list | Production build failed |
| Validation duplicated back into the draft path | M2's unified rules ↔ an older copy | A draft could bypass a rule the apply form enforced |

**Lesson recorded for the report.** Neither the production build nor any
member's test suite could see the first two classes of defect. A green build
proves only that the code *compiles*; an undefined identifier in JavaScript is an
error only when its line actually runs. A scope-aware checker
...
6. **Leave swap** — two employees swapping approved leave dates end-to-end through the approval chain (set this up before the demo: it needs two teammates with approved future leave, or the queue is empty)
6b. **Return early** — shorten an approved five-day leave to two days and watch exactly three days return to the balance, with the approver queue showing it as an *early return* rather than a cancellation
6c. **HR correction** — try to cancel leave that has already started, be told to ask HR, then switch to HR and adjust it; the days return and the reason lands on the audit trail
...
11b. **AI resilience** — unset the API key and re-run AI-1: it falls back to the offline parser and still fills the form. The system never depends on a provider being up
11c. **Certificates outstanding** — HR's compliance list surfaces sick leave still missing a required MC
...
- **HOD role:** Not built. Four roles were delivered (Employee, Supervisor, Manager, HR Admin) — a third approval tier added nothing to a two-tier workflow.
- **Reporting lines:** Not implemented. Approval routing is team-based plus delegation; the helper that would consume `supervisor_id` / `manager_id` exists and falls back to team scope.
- **Database:** MySQL 8, not PostgreSQL. Schema is created by `sequelize.sync({ alter: true })` rather than migration files — appropriate for five people adding columns concurrently, and the wrong choice for production.
- **Document storage:** Certificates are held in-row as base64 rather than in a file-storage service, so the app stays self-contained. Their declared file type is taken from the browser and **not** verified against the file's own bytes — a known gap.
- **Cross-year leave:** Leave spanning 31 December is charged entirely to the starting year. No split was implemented.
- **Self-declaration threshold:** The "certificate expected beyond 2 consecutive sick days" rule is a constant in code, not per-country configuration.
- **Scheduled jobs:** Implemented with `setInterval` on Singapore time rather than `node-cron`. Every job is idempotent, so a missed or repeated tick is harmless.
- **AI rate limiting:** Not implemented. Any authenticated user can drive provider spend.
